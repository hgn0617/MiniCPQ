...
2. 在“汇率管理”中维护外币汇率（定义为 1 单位外币兑换多少人民币）。
3. 在“服务器配置”中创建服务器型号并选择所需材料和数量。同类不同型号合计最多配置 1 个 CPU、4 个内存、3 个硬盘和 1 个主板，网卡可选。
4. 退出管理员账号，使用 `sales` 登录，选择客户和报价币种并创建报价。
5. 向报价中添加服务器、修改数量，然后提交报价。
6. 再次使用 `admin` 登录，设置毛利率或人民币售价并审批报价，系统同时显示本币和外币金额。
